...
from models.cavmunet.CaVMamba import VMUNet
...
def main(config):
    ...
    if config.network == 'cavmunet':
        ...
        model.load_from()
        # cal_params_flops(model.cuda(), 224, logger)
    else: raise('Please prepare a right net!')

    if config.distributed:
        model = torch.nn.SyncBatchNorm.convert_sync_batchnorm(model).cuda()
        model = DDP(model, device_ids='1', output_device='1')
    else:
